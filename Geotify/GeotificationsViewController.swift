...
class GeotificationsViewController: UIViewController, AddGeotificationsViewControllerDelegate, MKMapViewDelegate, CLLocationManagerDelegate {
  ...
  func mapView(_ mapView: MKMapView!, viewFor annotation: MKAnnotation!) -> MKAnnotationView! {
    ...
    if annotation is Geotification {
      ...
      if annotationView == nil {
        ...
        let removeButton = UIButton.buttonWithType(.custom) 
  ...
  if !CLLocationManager.isMonitoringAvailable(for: CLCircularRegion) {
    ...
    showSimpleAlertWithTitle("Warnings..", message: "Your geotification is saved but will only be activated once you grant Geotify permission to access the device location.", viewController: self)
    }
    // 3
